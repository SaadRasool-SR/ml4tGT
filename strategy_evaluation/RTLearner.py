...
class RTLearner(object):  		  	   		  		 			  		 			     			  	 
    ...
    def build_tree(self,data_x, data_y):
        ...
        if data_x.shape[0] <= self.leaf_size:
            ...
            return np.array([self.leaf_factor,stats.mode(data_y)[0], np.nan, np.nan])
            

        if np.all(data_y == data_y[0]):
            return np.array([self.leaf_factor, stats.mode(data_y[0])[0], np.nan, np.nan])


        else:
            #split_feature = self.get_corellation(data_x,data_y)

            split_feature = self.get_random_feature(data_x)
            #print(split_feature)
